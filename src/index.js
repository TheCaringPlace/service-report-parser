...
program
  ...
  .action(async ({ input, output }) => {
    ...
    for (const file of files) {
      ...
      const targetPath = join(output, file.replace(input + "/", "").replace(".pdf", ".txt"));
  ...
  .action(async ({ input, output }) => {
    ...
    for (const file of files) {
      ...
      if(report.reportType == null) {
        console.warn(`Skipping report ${file} because it could not be identified`);
        continue;
      }
      const targetPath = join(output, normalize(file).replace(normalize(input), "").replace(".pdf", ".json"));
...
program
  ...
  .requiredOption(
    ...
    "The file to save the consolidated JSON to",
  )
  .action(async ({ input, output }) => {
    const files = await crawlDirectory(input);
    ...
    await writeFileWithMkdir(output, JSON.stringify(consolidatedReports, null, 2));
  });
